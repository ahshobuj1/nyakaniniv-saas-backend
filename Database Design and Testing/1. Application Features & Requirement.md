...
- **DJ-to-Client Payments (Booking Payments):**
  - DJ can trigger a payment request to a client via email.
  - Two Payment Modes:
    1.  **Online (Stripe):** DJ sends a Stripe checkout link via email.
    2.  **Cash on Concert:** Client pays in person; DJ manually marks the payment as "Completed" in the dashboard.
- **Invoicing:**
  - **DJs:** Can view their Subscription Invoices and Booking Payments (from clients).
  - **Super Admin:** Can view all Subscription Invoices and individual DJ Booking Payments for auditing.

#### 4.5 Client Management (CRM)

- **Clients:**
  - A mini-CRM for DJs to manage their customers.
  - Each booking is linked to a specific client profile.
...
- **Bookings:** Client reference, Date, Status.
- **Subscription Invoices:** Transaction IDs, Amounts, Status (Paid/Unpaid), Date, Plan.
- **Booking Payments:** Amount, Method (Stripe/Cash), Status.
